...
import { capitalizeFirstLetter } from "libs/ultility";
...
const BlogCategoryPage = (props) => {
    ...
    React.useEffect(() => {
        if (categoryId){
            ...
            let isValidCategory = BlogCategories.find(cat => cat.toLowerCase() === (categoryId.toLowerCase()));
    ...
    const onCategory = (category: string) => (e) => {
        if (!categoryId || category === capitalizeFirstLetter(categoryId)){
            return;
        }
    ...
    const onPost = (post: TitlePost) => (e) => {
        ...
        router.push("/blog/" + categoryId.toLowerCase() + "/" +post.id + "-_" + title);
    }
    ...
    return <SLayout>
        <div className = {styles.BlogPage}>
            <div className = {styles.BlogHeader}>
                <h1><a href = {host + "/blog/"}><ArrowLeftOutlined /></a>{capitalizeFirstLetter(categoryId)}</h1>
            ...
            <div className = {styles.menu}>
                <div>
                    ...
                    <List
                        ...
                        renderItem={item => <List.Item className = {classNames(styles.Item, {[styles.ItemActive]: item.toLowerCase() === (categoryId.toLowerCase())})} onClick = {onCategory(item)}>{item}</List.Item>}
                    />
